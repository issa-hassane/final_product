    ...
    .then((response) => {
      if (response.length > 0) {
        ...
        res
          .status(200)
          .json({
            message: "user Logedin unSuccesful",
      ...
      if (response.length >= 1) {
        ...
        usersignedin
          ...
          .then((response) => {
            res
              .status(200)
              .json({ message: "user registerd Succesfully", user: response });
          })
          .catch((err) => {
            res.status(500).json({ error: err });
          });
...
exports.getuserById = (req, res) => {
  ...
  user
    ...
    .then((response) => {
      res
        .status(200)
        .json({ message: "user Fetched Succesfully", user: response });
    })
    .catch((err) => {
      res.status(500).json({ error: err });
    });
